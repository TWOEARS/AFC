% same as afc_win with modified background color defined in colorVec

function exampleCustomized_win(action)
		...
		switch def.markinterval
	   	case 1
			...
			for i=1:def.intervalnum-1
	   			...
	   			pause(inter)
	   			if ( ~isempty(h) )
					set(h,'backgroundcolor',colorVec)
	   			end	
	   			pause(paus)
			end
...
case 'markpressed'
   ...
   if ( ~isempty(hb) )
   	...
   	pause(0.2);
	set(hb,'backgroundcolor',colorVec);
	pause(0.0001);
   end
