% exampleCustomized_cfg - example measurement configuration file -
%
...
def.afcwin = 'exampleCustomized_win';	% custom response window

% save paths and save function
def.result_path = '';				% where to save results
def.control_path = '';				% where to save control files
def.savefcn = 'exampleCustomized';  % function which writes results to disk, here use custom savefcn
...
% sound command
def.externSoundCommand = 'exampleAudioplayer';  % use example custom sound command \addons\sound_exampleAudioplayer.m

% external hardware
def.extern_hardware = 'extern_exampleTDT';
